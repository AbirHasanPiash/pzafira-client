...
function DailySalesChart() {
  ...
  api.get('adminuser/api/daily-sales/').then(res => {
    const transformed = res.data.map(item => {
      ...
      return {
        date: isToday ? `${currentDate.format('D MMM')} (Today)` : currentDate.format('D MMM'),
    ...
    <div className="bg-white p-4 rounded-xl shadow-md relative">
      ...
      <ResponsiveContainer width="100%" height={300}>
        {isLineChart ? (
          ...
          <BarChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 5 }} style={{ backgroundColor: '#ffffff' }}>
            <CartesianGrid strokeDasharray="4 4" stroke="#e0e0e0" />
            <XAxis dataKey="date" angle={-30} textAnchor="end" height={60} />
            <YAxis allowDecimals />
            <Tooltip
