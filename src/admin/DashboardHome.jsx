...
const DashboardHome = () => {
  ...
  if (state.loading) return <p className="text-center p-8 text-gray-500">Loading dashboard...</p>;
  if (state.error) return <p className="text-center p-8 text-red-500">Error: {state.error}</p>;
      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="bg-white shadow-md p-6 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-semibold">৳{state.salesThisMonth.toLocaleString()}</p>
          </div>
        </div>
      </div>
  ...
  <div className="md:col-span-2"><DailyOrderChart /></div>
  <div className="md:col-span-2"><DailySalesChart /></div>

  {/* Monthly charts - side by side */}
  <div><MonthlyOrderChart /></div>
  <div><MonthlySalesChart /></div>
</div>
      </section>
    </div>
  );
